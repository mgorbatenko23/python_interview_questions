...
Функция создается однажды при загрузке модуля. Именованные параметры и их дефолтные значения тоже создаются один раз и хранятся в одном из полей объекта-функции в поле `__defaults__`, это кортеж.
...
- [Визуальный отладчик python](https://pythontutor.com/)
...
- **Скрытие переменных**  
  ...
  Если хотим изменить во вложенной функции переменую окружающей функции, то необходимо использовать ключевое слово **nonlocal**.
...
models.Model ---> def full_clean()  
                    ...
                    |-----------> def clean()- hook/custom            |---------> def to_python()
                    |-----------> def validate_unique                 |---------> def validate()
                    |-----------> def validate_constrains - custom    |---------> def run_validator()
                                                                                          |----------> validators - custom
```
hook/custom - то, где можно написать свой код для проверки модели/поля

**Проверка модели состоит из четырех этапов:**
1. Проверьте поля модели – Model.clean_fields()
2. Подтвердить модель в целом – Model.clean()
3. Подтвердите уникальность поля – Model.validate_unique()
...
Когда вы используете ModelForm, вызов is_valid() выполнит эти шаги проверки для всех полей, включенные в форму.


## Сохранение модели
Когда вы сохраняете объект, Django выполняет следующие шаги:  
1. **Подать сигнал предварительного сохранения**. pre_save сигнал отправляется, позволяя любым функциям, прослушивающим этот сигнал, выполнять что-то.

2. **Предварительная обработка данных**. Каждое поле pre_save() метод вызывается для выполнения любого необходимая автоматическая модификация данных. Например, поля даты/времени переопределить pre_save() для реализации auto_now_add и auto_now.

3. **Подготовьте данные для базы данных**. Каждое поле get_db_prep_save() методу предлагается предоставить его текущее значение в типе данных, который может быть записан в базу данных.  
   ...
   Например, поля DateField используют Python datetime объект для хранения данных. Базы данных не хранят datetime объекты, поэтому значение поля необходимо преобразовать в дату, соответствующую стандарту ISO. 

5. **Вставьте данные в базу данных**. Предварительно обработанные, подготовленные данные составлен в оператор SQL для вставки в базу данных.

6. **Подать сигнал после сохранения**. post_save сигнал отправляется, позволяя любым функциям, прослушивающим этот сигнал, выполнять что-то.
